...
export default function ActivityCard({
  ...
  // NOTE: Render straight from the `activity` prop. We used to hold a
  // local `activity` state initialized from the prop, but that's
  // the classic React anti-pattern: when the parent updated the
  // itinerary (e.g. via the chat refinement panel), the new activity
  // prop flowed down but the local state stayed pinned to its initial
  // value, so the card visibly didn't change even though state had
  // updated. Lift the swap to the parent via `onActivityChange` and the
  // new prop will re-render this card naturally.
          ...
          <div className="flex items-center gap-1.5 text-caption text-charcoal-800/40 font-sans">
            ...
            {activity.walkingTime && (
              <>
                ...
                <span>{activity.walkingTime}</span>
              </>
            )}
          </div>
        </div>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start gap-2 mb-1">
              <CurrentIcon
                className={cn("w-4 h-4 mt-1 shrink-0", config.color)}
              />
